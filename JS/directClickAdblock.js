...
window.gnoh = Object.assign(window.gnoh || {}, {
  ...
  dialog: function (title, content, buttons, config) {
    ...
    function onClickCloseDialog(event) {
      if (!event.target.closest('.dialog-custom[data-dialog-id="' + id + '"]')) {
        closeDialog(true);
      }
    }
...
(function () {
  ...
  function showDialogImportNew(blockList) {
    if (blockList.last_fetch_result === 'success') {
      ...
      gnoh.dialog("Import New Blocker List", importNewBlockerListHTML, [buttonOkElement, buttonCancelElement], {
        width: 500,
    ...
    vivaldi.contentBlocking.onRulesFetchComplete.removeListener(showDialogImportNew);
  }
      ...
      if (tab.windowId === vivaldiWindowId) {
        vivaldi.contentBlocking.addKnownSourceFromURL(vivaldi.contentBlocking.RuleGroup.AD_BLOCKING, blockListURL, function (blockListId) {
          if (!chrome.runtime.lastError) {
            vivaldi.contentBlocking.onRulesFetchComplete.addListener(showDialogImportNew);
          } else {
